...
def generate_pdf_report(target, vulnerabilities, exploits=None, scan_id=None):
    """Generate a PDF report using weasyprint. Falls back to HTML if weasyprint unavailable."""
    import os
    from datetime import datetime
...
body {{ font-family: 'Inter', 'Helvetica', sans-serif; color: #1a1a1a; padding: 20px; max-width: 100%; }}
h1 {{ color: #e0650a; font-size: 26px; border-bottom: 3px solid #e0650a; padding-bottom: 10px; }}
h2 {{ color: #e0650a; font-size: 16px; margin-top: 24px; border-bottom: 1px solid #ddd; padding-bottom: 6px; }}
.card {{ background: #f9f9f9; border: 1px solid #ddd; border-radius: 6px; padding: 14px; margin: 10px 0; }}
.badge {{ display: inline-block; padding: 2px 8px; border-radius: 8px; font-size: 10px; font-weight: 700; margin: 0 3px; }}
...
th {{ text-align: left; padding: 8px 6px; border-bottom: 2px solid #ddd; color: #666; font-size: 9px; text-transform: uppercase; background: #fafafa; }}
...
<p><strong>Scan ID:</strong> {scan_id or 'N/A'}</p>
<p><strong>Generated:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>

<h2>Executive Summary</h2>
<div class="card">
<p><strong>{len(vulnerabilities)} vulnerabilities discovered:</strong></p>
<p>
...
<td style="font-family:monospace;font-size:9px;">{v.get('endpoint','N/A')}</td>
<td>{v.get('parameter','N/A')}</td>
<td style="font-size:9px;">{v.get('description','')}</td>
</tr>"""
...
<div class="card"><ol>"""
    ...
    for v in vulnerabilities[:10]:
        ...
        html += f"<li><strong>{vtype}:</strong> {v.get('description','Fix the vulnerability')}</li>"
...
<p>FOR AUTHORIZED SECURITY TESTING ONLY</p>
    ...
    # Save HTML
    html_path = os.path.join(Config.REPORT_FOLDER, f'{report_id}.html')
    ...
    try:
        ...
        return {'success': True, 'pdf_path': pdf_path, 'html_path': html_path, 'report_id': report_id, 'format': 'pdf'}
    except ImportError:
        return {'success': True, 'html_path': html_path, 'report_id': report_id, 'format': 'html', 'note': 'Install weasyprint for PDF support'}
    except Exception as e:
        return {'success': True, 'html_path': html_path, 'report_id': report_id, 'format': 'html', 'note': f'PDF generation failed: {str(e)}'}
